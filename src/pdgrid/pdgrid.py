...
def pre_aggregation_pagination(df, groups, request):
    '''Only aggregate the columns needed to perform sorting and pagination to filter out unneeded rows before the heavy lifting'''
...
def sort_and_aggregate(df, request, response):
    ...
    if request.aggregation_group:
        ...
        if pagination := request.end_row < last_row or request.start_row > 0 or request.sort_agg_params:
            df, sorted_keys = pre_aggregation_pagination(df, groups, request)
